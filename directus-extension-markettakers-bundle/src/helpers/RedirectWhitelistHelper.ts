...
export class RedirectWhitelistHelper {
  static isRedirectUrlAllowedForWhitelistEntriesWithSimpleWildcards(
    redirect_whitelist_entries: string[],
    redirect_url: URL
  ) {
    for (let i = 0; i < redirect_whitelist_entries.length; i++) {
      ...
      try {
        if (
          RedirectWhitelistHelper.isRedirectUrlAllowedForWhitelistEntry(
  ...
  private static isRedirectUrlAllowedForWhitelistEntry(
    ...
    if (!hasProtocol) {
      ...
      let redirect_whitelist_entry_https =
        "https://" + redirect_whitelist_entry;
      // then we recursively call this function with the new URLs
      let redirect_allowed_http =
        RedirectWhitelistHelper.isRedirectUrlAllowedForWhitelistEntry(
          redirect_whitelist_entry_http,
          redirectUrl
        );
      let redirect_allowed_https =
        RedirectWhitelistHelper.isRedirectUrlAllowedForWhitelistEntry(
      ...
      if (redirectUrlProtocol.includes(WILDCARD_REPLACEMENT)) {
        ...
        const protocolMatches =
          RedirectWhitelistHelper.startsWithUntilWildcardReplacement(
            redirectUrlProtocol,
  ...
  private static startsWithUntilWildcardReplacement(
    inputStr: string,
